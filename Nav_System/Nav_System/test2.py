...
def Check_Time_Memory(memory,time_count):
	result =[]
	for key in memory:
		result.append(key)
	...
	for name in result:
		if len(memory[str(name)]["time_memory"]) == time_count:
			memory[str(name)]["state"] = True
...
def Main():
	...
	while True:
		...
		memory = Check_Time_Memory(memory,time_count)
